...
func (q *Queries) CreateNewSession(ctx context.Context, arg CreateNewSessionParams) (Session, error) {
	row := q.queryRow(ctx, q.createNewSessionStmt, createNewSession,
		arg.ID,
		arg.Username,
		arg.RefreshToken,
		arg.UserAgent,
...
const getSesion = `-- name: GetSesion :one
SELECT id, username, refresh_token, user_agent, client_ip, is_blocked, expires_at, created_at FROM sessions
WHERE id = $1 LIMIT 1
`

func (q *Queries) GetSesion(ctx context.Context, id uuid.UUID) (Session, error) {
	row := q.queryRow(ctx, q.getSesionStmt, getSesion, id)
	var i Session
	err := row.Scan(
		&i.ID,
		&i.Username,
